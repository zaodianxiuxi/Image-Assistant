...
- 诗词意境工作台：理解诗词情绪与意象，拆成画面提示词后逐段或批量生成图片
- 诗词项目持久化：原文、意境分析、分镜、补充提示词和画幅保存到 MySQL，刷新后可继续编辑
...
诗词画面默认分为 6 段，也可以选择 3–8 段。点击“分析诗词意境”后，文本模型会结合季节、时辰、人物、空间、情绪和核心意象，输出结构化分析；确认或修改分析内容后，再点击“生成分镜提示词”。

生成后的提示词可以逐段修改。点击某一段右侧的生成按钮只生成该段，点击“批量生成全部”则按当前顺序生成所有段落；每段都会显示等待、生成中、成功或失败状态，批量生成还会显示当前序号和进度条。遇到限流、网关或服务暂时不可用时会自动重试，仍失败的段落可以点击“重新生成”。结果会回填到主画布并保存到本地图库。图片画幅沿用主工作台当前选择。
...
诗词项目会额外写入 `poetry_projects` 表，并通过 `series_id` 关联图片合集；修改项目时更新同一条记录，不会因为刷新或继续编辑重复创建项目。应用启动迁移时，会把旧的“诗词意境创作合集”自动补成可继续编辑的诗词项目。
